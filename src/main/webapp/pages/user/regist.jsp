	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#code_img").click(function () {
				//在事件响应的function函数中有一个this对象。这个this对象，是当前正在响应事件的dom对象，相对于该事件的标签
			...
			$("#sub_btn").click(function(){
				...
				//2、将确认密码文本框中的内容域密码文本框中的内容进行比较
				if (repwdText != passwordText){
					//4、提示用户结果
					$("span.errorMsg").text("确认密码和密码不匹配");
					return false
				}
				// 验证邮箱：xxxx@xxx.com
				//1、获取邮箱文本框中的内容
				...
				if ($.trim(codeText) == null || $.trim(codeText) ==""){
					//4、提示用户结果
								...
								<form action="userServlet" method="post">
									...
									<input class="itxt" type="text" placeholder="请输入用户名" autocomplete="off"
										   tabindex="1" name="username" id="username"
											value="${requestScope.username}"/><%--<%=request.getAttribute("username")==null?"":request.getAttribute("username")%>--%>
									<br />
									<br />
									<label>用户密码：</label>
									...
									<input class="itxt" type="text" placeholder="请输入邮箱地址" autocomplete="off"
										   tabindex="1" name="email" id="email"
											value="${requestScope.email}"/><%--<%=request.getAttribute("email")==null?"":request.getAttribute("email")%>--%>
									<br />
									<br />
									<label>验证码：</label>
